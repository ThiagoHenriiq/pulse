...
export default function Onboarding({ onFinish }: { onFinish: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-neutral-900 animate-fade-in p-4">
      <img src={steps[step].img} alt="Onboarding" className="w-24 h-24 mb-6 rounded-xl shadow-lg" />
      <h2 className="text-2xl font-bold text-white mb-2 text-center">{steps[step].title}</h2>
      <p className="text-neutral-400 text-base mb-6 text-center">{steps[step].desc}</p>
      {step < steps.length - 1 ? (
